...
import com.btg.pactual.fondos.domain.exceptions.InscripcionNotFoundException;
...
public class InscripcionService {
    ...
    public Inscripcion crearInscripcion(Inscripcion inscripcion) throws ClienteNotFoundException {
        Cliente cliente = clienteRepository.findById(inscripcion.getCliente().getIdCliente())
                .orElseThrow(() -> new ClienteNotFoundException(
                        "Cliente con ID: " + inscripcion.getCliente().getInscripciones() + " no encontrado."));
        return inscripcionRepository.save(inscripcion);
    }

    public Inscripcion actualizarInscripcion(String idInscripcion, Inscripcion inscripcionActualizada)
    ...
    public void cancelarInscripcionDeCliente(String idCliente, String idInscripcion)
            throws ClienteNotFoundException, InscripcionNotFoundException {
        Cliente cliente = clienteRepository.findById(idCliente)
                .orElseThrow(() -> new ClienteNotFoundException("Cliente con ID: " + idCliente + " no encontrado."));

        boolean inscripcionExiste = cliente.getInscripciones().removeIf(
                inscripcion -> inscripcion.getIdInscripcion().equals(idInscripcion));
